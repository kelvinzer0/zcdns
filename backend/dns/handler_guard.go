...
var defaultServerIPv6 = net.ParseIP("2606:c700:4020:0098:1234:4321:73ab:0001")

func (s *Server) handleGuardDomain(m *dns.Msg, q dns.Question, name string) {
	s.respondWithServerIPs(m, q, name)
}

func (s *Server) handleRouterDomain(m *dns.Msg, q dns.Question, name string) {
	s.respondWithServerIPs(m, q, name)
}

func (s *Server) respondWithServerIPs(m *dns.Msg, q dns.Question, name string) {

...
export function AdminPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || t("login-error-default"));
      }
      ...
      setLoginError(err.message || t("login-failed"));
    } finally {
  ...
  const handleUpdateStatus = async (
    ...
    try {
      ...
      if (res.ok) {
        const sub = reports.find((r) => r.id === id)?.subdomain || "";
        setActionSuccess(
          blockSubdomain
            ? t("success-blocked", { subdomain: sub })
            : t("success-status-updated", { status })
        );
        fetchAdminData();
  ...
  const handleDeleteReport = async (id: string) => {
    if (!adminToken) return;
    if (!confirm(t("confirm-delete-report"))) return;
    ...
    try {
      ...
      if (res.ok) {
        setActionSuccess(t("success-report-deleted"));
        fetchAdminData();
      }
    } catch (err) {
  ...
  const handleManualBlock = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setActionSuccess(t("success-blocked", { subdomain: manualSubdomain }));
        setManualSubdomain("");
        setManualReason("");
        fetchAdminData();
      }
  ...
  const handleUnblock = async (subdomain: string) => {
    if (!adminToken) return;
    if (!confirm(t("confirm-unblock", { subdomain }))) return;
    ...
    try {
      ...
      if (res.ok) {
        setActionSuccess(t("success-unblocked", { subdomain }));
        fetchAdminData();
      }
    } catch (err) {
  ...
  if (!adminToken) {
    return (
      <main className="pt-28 pb-20 px-4 sm:px-6 lg:px-8 min-h-screen bg-gray-50 flex items-center justify-center">
        ...
        />
        <div className="max-w-md w-full bg-white rounded-none shadow-none border border-gray-200 p-8">
          <div className="text-center mb-6">
            <div className="w-12 h-12 bg-red-100 rounded-none flex items-center justify-center mx-auto mb-3">
          ...
          {loginError && (
            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-none text-xs text-red-700">
              {loginError}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-none border border-gray-200 focus:outline-none focus:ring-2 focus:ring-red-500 text-sm"
              />
            </div>
            <button
              type="submit"
              disabled={isLoggingIn}
              className="w-full py-2.5 bg-[#012241] hover:bg-[#02365f] text-white font-medium rounded-none text-sm transition-all flex items-center justify-center gap-2 shadow-none disabled:opacity-50"
            >
              {isLoggingIn ? t("login-processing") : t("login-button")}
            </button>
          </form>
        </div>
      </main>
  ...
  return (
    <main className="pt-24 pb-20 px-4 sm:px-6 lg:px-8 min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto max-w-7xl">
        {/* Top Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-none border border-gray-200 shadow-none mb-8">
          <div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={isLoading}
              className="p-2.5 bg-gray-100 hover:bg-gray-200 rounded-none text-gray-700 transition-colors"
        ...
        {/* Success Alert */}
        {actionSuccess && (
          <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-none flex items-center justify-between text-green-800 text-sm">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {["all", "pending", "investigating", "resolved_blocked", "dismissed"].map((st) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-none text-xs font-semibold capitalize transition-colors ${
                      statusFilter === st
                      ...
                      return (
                        <tr key={rep.id} className="hover:bg-gray-50/80 transition-colors">
                          ...
                          <td className="py-3.5 px-4 font-mono font-bold text-gray-900">
                            ...
                            {isBlocked && (
                              <span className="ml-2 px-1.5 py-0.5 bg-red-100 text-red-700 text-[10px] rounded-none font-sans uppercase">
                                {t("badge-blocked")}
                              </span>
                            )}
                          </td>
                          <td className="py-3.5 px-4">
                            <span className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded-none text-xs capitalize font-medium">
                              {rep.abuse_type}
                            </span>
                          </td>
                          <td className="py-3.5 px-4 text-xs">
                            <div className="font-semibold text-gray-900">{rep.reporter_name || t("reporter-anonymous")}</div>
                            <div className="text-gray-500">{rep.reporter_email}</div>
                          </td>
                          <td className="py-3.5 px-4">
                            <span className={`px-2.5 py-1 rounded-none text-xs font-semibold border ${badgeClass}`}>
                              {rep.status}
                            </span>
                          </td>
                          <td className="py-3.5 px-4 text-right space-x-1 whitespace-nowrap">
                            <button
                              ...
                              className="p-1.5 text-gray-600 hover:bg-gray-100 rounded-none"
                            ...
                            {rep.status !== "investigating" && (
                              <button
                                ...
                                className="px-2 py-1 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-none text-xs font-medium border border-blue-200"
                              >
                                {t("btn-investigate")}
                              </button>
                            )}
                            ...
                            {!isBlocked && (
                              <button
                                ...
                                className="px-2 py-1 bg-red-600 text-white hover:bg-red-700 rounded-none text-xs font-semibold shadow-none"
                              >
                                {t("btn-block-subdomain")}
                              </button>
                            )}
                            ...
                            {rep.status !== "dismissed" && (
                              <button
                                ...
                                className="px-2 py-1 bg-gray-100 text-gray-700 hover:bg-gray-200 rounded-none text-xs font-medium"
                              >
                                {t("btn-dismiss")}
                              </button>
                            )}
                            ...
                            <button
                              ...
                              className="p-1.5 text-red-500 hover:bg-red-50 rounded-none"
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-none border border-gray-200 shadow-none">
              ...
              <form onSubmit={handleManualBlock} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">{t("manual-block-subdomain-label")}</label>
                  <input
                    type="text"
                    required
                    placeholder={t("manual-block-subdomain-placeholder")}
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">{t("manual-block-reason-label")}</label>
                  <input
                    type="text"
                    placeholder={t("reason-placeholder")}
                    value={manualReason}
                    onChange={(e) => setManualReason(e.target.value)}
                    className="w-full px-3.5 py-2 rounded-none border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
                  />
                </div>
                <div className="flex items-end">
                  <button
                    type="submit"
                    className="w-full py-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-none text-sm transition-colors shadow-none flex items-center justify-center gap-2"
                  >
                    <Ban className="w-4 h-4" />
                    {t("btn-add-block")}
                  </button>
            ...
            <div className="bg-white rounded-none border border-gray-200 shadow-none overflow-hidden">
              <div className="p-4 border-b border-gray-100 bg-gray-50/50">
                <h4 className="font-bold text-gray-900 text-sm">{t("blocked-list-title")}</h4>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-gray-50 text-gray-500 text-xs uppercase font-semibold border-b border-gray-100">
                    <tr>
                      <th className="py-3 px-4">{t("col-subdomain")}</th>
                      <th className="py-3 px-4">{t("col-reason")}</th>
                      <th className="py-3 px-4">{t("col-blocked-date")}</th>
                        ...
                        <tr key={item.subdomain} className="hover:bg-gray-50">
                          ...
                          <td className="py-3.5 px-4 text-right">
                            <button
                              ...
                              className="px-3 py-1.5 bg-green-50 text-green-700 hover:bg-green-100 rounded-none text-xs font-semibold border border-green-200 transition-colors"
                            >
                              {t("btn-unblock")}
                            </button>
                          </td>
            ...
            <div className="bg-white p-6 rounded-none border border-gray-200 shadow-none">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center py-2 border-b border-gray-100">
                  <span className="text-gray-600 text-sm">{t("stats-active-subdomains")}</span>
                  <span className="font-mono font-bold text-gray-900 text-lg">
                    {growthStats.active_subdomains}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-100">
                  <span className="text-gray-600 text-sm">{t("stats-active-records")}</span>
                  <span className="font-mono font-bold text-gray-900 text-lg">
                    {growthStats.active_records}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-100">
                  <span className="text-gray-600 text-sm">{t("stats-total-queries")}</span>
                  <span className="font-mono font-bold text-gray-900 text-lg">
                    {growthStats.total_queries}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-100">
                  <span className="text-gray-600 text-sm">{t("stats-blocked-domains")}</span>
        ...
        {selectedReport && (
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-none max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-none border border-gray-200 p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <button
                  ...
                  className="p-2 text-gray-400 hover:text-gray-600 rounded-none"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="space-y-4 text-sm">
                <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-none border border-gray-100">
                  <div>
                    <span className="text-xs text-gray-500 block">{t("modal-reporter")}</span>
                  ...
                  <div>
                    <span className="text-xs text-gray-500 block">{t("modal-violation-type")}</span>
                    <span className="font-semibold text-red-600 capitalize">{selectedReport.abuse_type}</span>
                    <span className="text-xs text-gray-500 block">
                      {t("col-status")}: <strong className="uppercase">{selectedReport.status}</strong>
                    </span>
                  </div>
                </div>

                <div>
                  <span className="text-xs font-semibold text-gray-700 block mb-1">{t("modal-description")}</span>
                  <div className="p-3 bg-gray-50 rounded-none border border-gray-100 text-gray-800 whitespace-pre-wrap">
                ...
                {selectedReport.evidence && (
                  <div>
                    <span className="text-xs font-semibold text-gray-700 block mb-1">{t("modal-evidence")}</span>
                    <div className="p-3 bg-gray-50 rounded-none border border-gray-100 text-gray-800 whitespace-pre-wrap font-mono text-xs">
                      {selectedReport.evidence}
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-3 rounded-none border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
                  />
                </div>
                ...
                <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-100 justify-end">
                  <button
                    ...
                    className="px-4 py-2 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-none font-semibold text-xs border border-blue-200"
                  >
                    {t("btn-mark-investigating")}
                  </button>
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-red-600 text-white hover:bg-red-700 rounded-none font-semibold text-xs shadow-none flex items-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-gray-100 text-gray-700 hover:bg-gray-200 rounded-none font-medium text-xs"
                  >
                    {t("btn-dismiss-report")}
                  </button>
                </div>
              </div>
            </div>
